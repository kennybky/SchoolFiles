...
x_image = tf.reshape(x, [-1, img_size, img_size, num_channels], name='images')
...
def create_weights(shape, stddev):
...
def create_convolutional_layer(input,
    ...
    weights = create_weights(shape=[conv_filter_size, conv_filter_size, num_input_channels, num_filters], stddev=0.05)
...
def normalize(input):
    ...
    layer = tf.nn.local_response_normalization(input, depth_radius=4, bias=1, alpha=0.001/9.0, beta=0.75, name=None)
    return layer
...
def create_fc_layer(input,          
    ...
    weights = create_weights(shape=[num_inputs, num_outputs], stddev=0.04)
...
normal1 = normalize(layer_conv1)
max_pool1 = max_pool(normal1)



layer_conv2 = create_convolutional_layer(input=max_pool1,
